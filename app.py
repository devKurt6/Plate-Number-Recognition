...
# Assuming you have a function to check the user's role or permissions
...
def join():
    if request.method == 'POST':
        ...
        with sqlite3.connect("mydb.db") as users:
            ...
            if existing_user:
                return jsonify({'success': True, 'message': 'User already exists. Please choose a different name or plate number.'})
...
def check_plate_number(extracted_text_str):
    ...
    if result and result[1] == 0:  # Check if result exists and entry is 0
        ...
        current_datetime = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        cursor.execute('''INSERT INTO timeDate (TimeAndDate, Pnumber,Status,image) VALUES (?,?,?, ?)''',
                       (current_datetime, extracted_text_str,'Time in',image_binary))
        # Commit changes and close the connection
        conn.commit()
        cursor.execute("SELECT Vtype FROM car_user WHERE Pnumber = ?", (extracted_text_str,))
        vehicle_type = cursor.fetchone()[0]  # Fetch the vehicle type from the databass
